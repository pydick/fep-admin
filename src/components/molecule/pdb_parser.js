...
export async function check_pocket(pdb_file, delete_hets, center_dic, dim_dic) {
  ...
  const delete_hets_list = delete_hets.map(
    item => item.chain_id + ":" + item.residue_number
  );
  ...
  for (const line of lines) {
    if (line.startsWith("HETATM")) {
      ...
      if (
        ...
        ligand_dict[chain_name + ":" + ligand_name + ":" + residue_id].count +=
          1;
        ligand_dict[
          chain_name + ":" + ligand_name + ":" + residue_id
