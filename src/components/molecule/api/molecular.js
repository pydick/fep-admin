...
export function get_molfact_ref_lig(smiles_id) {
  return request.get(
    "/api/jobs/simple_job/molfact_ref_lig?smiles_id=" + smiles_id
  );
}
// 获取对接参考配体
...
export function get_docked_pdb_api(smiles_id, need_smiles = false) {
  if (need_smiles) {
    return request.get(
      "/api/jobs/simple_job/docking_result?need_smiles=true&smiles_id=" +
        smiles_id
    );
  } else {
    return request.get(
      "/api/jobs/simple_job/docking_result?smiles_id=" + smiles_id
    );
  }
}
...
export function get_smiles_detail(job_id, params) {
  return request.post(
    "/api/jobs/" + job_id + "/result/molgen_pipeline/smiles_detail",
    params
  );
}
...
export function download_data(job_id, params) {
  return request.post(
    "/api/jobs/" + job_id + "/result/molgen_pipeline/download",
    params
  );
}
