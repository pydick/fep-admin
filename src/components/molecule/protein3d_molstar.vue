<template>
  ...
  >
    <drugflow_molstar
      ref="molstar_ref"
      :if_sequence_panel="false"
      :disable_comp_btn="true"
      :show_tool="false"
      :prolif_show_ligname="false"
    />
  </div>
</template>
...
const props = defineProps({
  ...
  ligands_list: {
    ...
    default: () => [
      {
        ligand_hex_id: "sdasda",
        ligand_smiles: "CCCCCCC",
        ligand_show_name: "1",
        pdb_string: "",
        residue_full_info: []
      },
      {
        ligand_hex_id: "sa",
        ligand_smiles: "CCCCCCCNNN",
        ligand_show_name: "2",
        pdb_string: "",
        residue_full_info: []
      },
      {
        ligand_hex_id: "23dasda",
        ligand_smiles: "C1CCCCCC1",
        ligand_show_name: "3",
        pdb_string: "",
        residue_full_info: []
      }
...
const draw = async () => {
  try {
    ...
    const ligands_hex_id_list = props.ligands_list.map(
      item => item.ligand_hex_id
    );
    // 先移除own_ligands_hex_id_list多出来的id, 并更改color cnt
    for (let i = 0; i < own_ligands_hex_id_list.value.length; i++) {
      if (
        ligands_hex_id_list.indexOf(own_ligands_hex_id_list.value[i]) === -1
      ) {
        change_color_cnt(own_ligands_list.value[i].color, -1);
        // remove ligand view
        await molstar_ref.value.remove_ligand_view_pdb(
          own_ligands_list.value[i].id
        );
    ...
    for (let i = 0; i < ligands_hex_id_list.length; i++) {
      if (
        own_ligands_hex_id_list.value.indexOf(ligands_hex_id_list[i]) === -1
      ) {
        ...
        // add ligand view
        await add_ligand_view(
          props.ligands_list[i].pdb_string,
          ligands_hex_id_list[i],
          i,
          false,
          props.ligands_list[i].ligand_show_name,
          color,
          props.ligands_list[i].residue_full_info
        );
      }
    }
    close_loading();
